...
def require_event_manager(user=Depends(get_current_user)):
    """Event creation/management: all staff roles except principal."""
    if user["role"] not in ("coordinator", "dean", "teacher", "registrar", "admin"):
        raise HTTPException(403, "Event management requires staff role")
    return user
